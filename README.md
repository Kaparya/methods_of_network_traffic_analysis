...
1. **NLP признаки ("лет", "года")**: Слова, указывающие на длительность опыта в тексте.
...
1. **Синтетическая разметка**: Целевая переменная (`y`) формируется на основе эвристических правил (должность + стаж), а не экспертной оценки HR. В пограничных случаях (например, "Strong Junior" vs "Middle") эвристика может ошибаться, а модель может находить более сложные паттерны, которые формально считаются ошибкой относительно "шумной" разметки.

...
│   ├── utils/              # Вспомогательные функции (I/O, графики, отчеты)
│   └── handlers/           # Логика обработки данных
│       ├── filtering.py    # Фильтр IT-вакансий
│       ├── labeling.py     # Логика разметки Junior/Middle/Senior
│       ├── parsing/        # Директория с парсерами признаков
│       ├── preprocessing/  # Кодирование и разбиение данных
│       └── io/             # Загрузка и сохранение данных
...
- Python 3.9+
- RAM: минимум 4 ГБ (для обработки датасета hh.ru)
...
2. Убедитесь, что файл `hh.csv` находится в директории `parsing/` (или в корне проекта).
...
### Точка входа
Основной скрипт для запуска классификации:
```bash
python3 poc_script.py
```

### Тестирование
В текущей версии PoC тестирование производится на отложенной выборке (20% данных), результаты выводятся в консоль и сохраняются в `classification_report.txt`.
